...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function showHomeGreetingPopup() {
        if (localStorage.getItem('home-greeting-dismissed') === 'true') return;

        const path = window.location.pathname.replace(/^\/|\/$/g, '');
        const isHome = path === '' || path === 'index.html';
        if (!isHome) return;

        const modal = document.getElementById('home-greeting-modal');
        if (!modal) return;

        const close = () => modal.classList.remove('active');

        document.getElementById('home-greeting-ok')?.addEventListener('click', close, { once: true });
        document.getElementById('home-greeting-dont-show')?.addEventListener(
            'click',
            () => {
                localStorage.setItem('home-greeting-dismissed', 'true');
                close();
            },
            { once: true }
        );
